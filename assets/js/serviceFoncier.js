...
var url = geoserverURL + "/geoserver/landplateform/wms"
var HydroligneSource = new ol.source.TileWMS({
  url: url,
  params: {
    'FORMAT': "image/png",
    'VERSION': '1.1.1',
    'LAYERS' : 'landplateform:hydrographie_lineaire',
    'TILED' : true,
...
var HydrozoneSource = new ol.source.TileWMS({
  ...
  params: {
    'FORMAT': "image/png",
    'VERSION': '1.1.1',
    'LAYERS' : 'landplateform:hydrographie_zonale',
    'TILED' : true,
    "exceptions": 'application/vnd.ogc.se_inimage',
...
var limCommuneSource = new ol.source.TileWMS({
  ...
  params: {
    'FORMAT': "image/png",
    'VERSION': '1.1.1',
    'LAYERS' : 'landplateform:limite_commune',
    'TILED' : true,
    "exceptions": 'application/vnd.ogc.se_inimage',
...
var orthosource = new ol.source.TileWMS({
  url: url,
  params: {
    'LAYERS': 'landplateform:Antsirabe_I_II',
    'VERSION': '1.1.1',
    'TILED' : true,
    'FORMAT': 'image/png',
...
var pudisource = new ol.source.TileWMS({
  url: url,
  params: {
    'LAYERS': 'landplateform:pudi',
    'VERSION': '1.1.1',
    'TILED' : true,
    'STYLES': 'landplateform:pudi',
    'FORMAT': 'image/png',
    // 'FORMAT_OPTIONS': "layout:style-editor-legend;fontAntiAliasing:true",
    'RANDOM': -553447854,
    'LEGEND_OPTIONS': 'forceLabels:on;fontAntiAliasing:true',
...
var cadastresource = new ol.source.TileWMS({
  url: url,
  params: {
    'LAYERS': 'landplateform:cadastre',
    'VERSION': '1.1.1',
    'TILED' : true,
    // 'STYLES': 'landplateform:cadastre',
    'FORMAT': 'image/png',
    // 'FORMAT_OPTIONS': "layout:style-editor-legend;fontAntiAliasing:true",
    'RANDOM': -553447854,
    'LEGEND_OPTIONS': 'forceLabels:on;fontAntiAliasing:true',
...
var cadastrelayer = new ol.layer.Tile({
...
var plofsource = new ol.source.TileWMS({
  url: url,
  params: {
    'LAYERS': 'landplateform:vw_plof',
    'STYLES': 'landplateform:vw_plof',
    'VERSION': '1.1.1',
    'FORMAT': 'image/png',
    // 'FORMAT_OPTIONS': "layout:style-editor-legend;fontAntiAliasing:true",
    'RANDOM': 48810829,
...
var certificatSource = new ol.source.TileWMS({
  url: url,
  params: {
    'LAYERS': 'landplateform:certificats',
    // 'STYLES': 'landplateform:vw_plof',
    'VERSION': '1.1.1',
    'FORMAT': 'image/png',
    // 'FORMAT_OPTIONS': "layout:style-editor-legend;fontAntiAliasing:true",
...
document.getElementById('draw-button').addEventListener('click', function() {
  ...
  if (map.getView().getZoom() > 7) {
    ...
    alert("Le zoom doit être supérieur à 10 pour pouvoir dessiner avec précision");
  }
});

// Retirer l'interaction de dessin si le zoom descend en dessous de 10
...
var urldemandeCadastre = geoserverURL+ "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20cadastre&CQL_FILTER=updated_or_new+=+%27true%27&outputFormat=application/json"; 
// var urldemandePermis = geoserverURL+ "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20permis&CQL_FILTER=updated_or_new+=+%27true%27&outputFormat=application/json"; 
...
// Afficher la couche en attente en jaune
var urldemandeDPE = geoserverURL+ "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20dpe&CQL_FILTER=updated_or_new+=+%27true%27&outputFormat=application/json"; 
// var urldemandePermis = geoserverURL+ "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20permis&CQL_FILTER=updated_or_new+=+%27true%27&outputFormat=application/json"; 
...
var dpeArectifierurl = geoserverURL + "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20dpe&CQL_FILTER=a_rectifier+=+%27true%27&outputFormat=application/json"; 
// var permisArectifierUrl = geoserverURL + "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20permis&CQL_FILTER=a_rectifier+=+%27true%27&outputFormat=application/json"; 
...
var CadastreRectifiergeojson = new ol.layer.Vector({
  source: new ol.source.Vector({
    url: dpeArectifierurl,
    format: new ol.format.GeoJSON(),
    wrapX: false,
  }),
...
map.addLayer(CadastreRectifiergeojson);

var dpeArectifierurl = geoserverURL + "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20dpe&CQL_FILTER=a_rectifier+=+%27true%27&outputFormat=application/json"; 
var dpeRectifiergeojson = new ol.layer.Vector({
  source: new ol.source.Vector({
    url: dpeArectifierurl,
    format: new ol.format.GeoJSON(),
    wrapX: false,
...
map.addLayer(dpeRectifiergeojson);

// Afficher la couche demandée refusée en rouge
var cadastreRefuseURL = geoserverURL + "/geoserver/landplateform/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=%20cadastre&CQL_FILTER=validee_publiee+=+%27false%27&outputFormat=application/json"; 
...
var CadastreRefuseegeojson = new ol.layer.Vector({
  source: new ol.source.Vector({
    url: cadastreRefuseURL,
    format: new ol.format.GeoJSON()
  }),
  style: RefuseeStyleFunction // Utilisez la fonction de style ici
});

map.addLayer(CadastreRefuseegeojson);
...
function updateNotification() {
  ...
  // Récupérer les demandes en attente
    $.ajax({
        ...
        data: {
            entite: 'serviceFoncier', status : 'attente'
        },
        type: 'get',
        dataType: 'json',
    ...
    $.ajax({
        url: '../gis/functions.php',
        data: {
            entite: 'serviceFoncier', status : 'acceptee'
        },
        type: 'get',
        dataType: 'json',
        success: function(response) {
    ...
    $.ajax({
        url: '../gis/functions.php',
        data: {
            entite: 'serviceFoncier', status : 'a_rectifier'
        },
        type: 'get',
        dataType: 'json',
        success: function(response) {
            ...
            $.each(response.slice(0, 5), function (index, data){
                ...
                a.on('click', function() { 
                  centerMapTo(lat, lon);
                  ShowFormRect(data.gid, data.couche, data.observation, data.numcf, data.surface)
                }); 
    ...
    $.ajax({
        url: '../gis/functions.php',
        data: {
            entite: 'serviceFoncier', status : 'refusee'
        },
        type: 'get',
        dataType: 'json',
        success: function(response) {
            ...
            $.each(response.slice(0, 5), function (index, data){
              addStarToUnseen(data, a)
                ...
                a.on('click', function() { 
                  centerMapTo(lat, lon);
                  showFormRefus(data.observation, data.gid, data.couche, data.numcf, data.numdemande);
                }); 
...
function NewNotification(couche) {
  $.ajax({
    ...
    type: 'POST',
    data: {
      action: 'newMessageSF', attribute : 'updated_or_new'
    },
    success: function(response) {
      var notificationNombre = $('#Attentenombre');
...
function ShowFormRect(gid, couche, remarque, numcf, surface) {
